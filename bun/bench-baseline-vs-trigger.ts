...
const TOTAL = 100_000;
const RUNS = 10;
const DB_PATH = "/tmp/bench-bun-trigger.db";

function bench(mode: "baseline" | "triggers"): number[] {
	const qpsList: number[] = [];
	...
	for (let run = 0; run < RUNS; run++) {
      ...
      END`);
		...
		const count = stmtCount.get() as { c: number };
		if (count.c !== TOTAL)
			throw new Error(`VERIFY FAIL: items=${count.c} expected=${TOTAL}`);

		if (mode === "triggers") {
			const cc = stmtChanges.get() as { c: number };
			if (cc.c !== TOTAL)
...
console.log("============================================");
console.log("Benchmark: baseline vs triggers (Bun + bun:sqlite)");
console.log(
...
for (const mode of ["baseline", "triggers"] as const) {
	console.log(`>>> ${mode}`);
