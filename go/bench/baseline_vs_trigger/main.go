...
func bench(mode string) []float64 {
	...
	for run := 0; run < runs; run++ {
		...
		for _, ext := range []string{"", "-wal", "-shm"} {
			os.Remove("/tmp/bench-go-trigger.db" + ext)
		}

		db, err := sql.Open("sqlite3", "/tmp/bench-go-trigger.db?_journal_mode=WAL&_synchronous=NORMAL&_busy_timeout=5000")
		if err != nil {
			log.Fatal(err)
		}
...
func main() {
	fmt.Println("============================================")
	fmt.Println("Benchmark: baseline vs triggers (Go + mattn/go-sqlite3)")
