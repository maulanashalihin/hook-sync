...
Multi-language SQLite replication via change capture + batched HTTP ship + UUID primary keys. Go uses `sqlite3_preupdate_hook` (zero overhead); Bun/Node use SQLite triggers (1 extra INSERT per change).
...
This is the reliable benchmark — pure SQLite write speed with capture mechanism firing.
...
Bun:sqlite binding is fastest in raw SQLite — even with trigger overhead (1 extra INSERT per change), it beats Go's preupdate_hook (zero overhead) in transaction mode. Go's mattn/go-sqlite3 binding has more overhead per call. Node's better-sqlite3 sits between.

### HTTP Benchmark (100 concurrent requests, 2 nodes same runtime)

⚠️ **High variance — interpret with caution.** Localhost HTTP benchmark with 100 requests shows 10x variance across runs. Numbers below are from a single run; do not treat as definitive runtime comparison.
...
Write latency and sync delay are comparable across all three runtimes. Bun wins read latency (bun:sqlite binding). Sync delay = batch interval (50ms) in all cases — bottleneck is the timer, not the runtime.
...
| Write latency p50 (HTTP) | **0.09ms** | 0.10ms | 0.10ms |
| Read latency p50 (HTTP) | 0.21ms | 0.31ms | **0.14ms** |
